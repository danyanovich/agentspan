...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/agents') return json(res, 200, Object.values(core.projections.snapshot().operational.agents));
  if (req.method === 'POST' && url.pathname === '/goals') return json(res, 201, core.createGoal(await readJson(req)));
  if (req.method === 'GET' && url.pathname === '/goals') return json(res, 200, Object.values(core.projections.snapshot().operational.goals));
  if (req.method === 'POST' && url.pathname === '/tasks') return json(res, 201, core.createTask(await readJson(req)));
  if (req.method === 'GET' && url.pathname === '/tasks') return json(res, 200, Object.values(core.projections.snapshot().operational.tasks));
  ...
  if (req.method === 'GET' && projectionOp) return json(res, 200, core.projections.snapshot().operational);

  const projectionSp = url.pathname.match(/^\/worlds\/([^/]+)\/projection\/spatial$/);
  if (req.method === 'GET' && projectionSp) return json(res, 200, core.projections.snapshot().spatial);
